...
THREE.TrackballControls = function ( object, domElement ) {
	...
	this.rotateCamera = (function(){
		...
		return function () {
			...
			if ( angle ) {
                ...
                var position = _this.object.position.clone();
                _this.object.up.applyQuaternion( quaternion );
                _eye.applyQuaternion( quaternion );
                _this.object.position.addVectors( _this.target, _eye );

                if((_this.object.up.z < 0) || (_this.object.position.z < 0 && _this.object.position.z < position.z))
                {
                    quaternion.setFromAxisAngle( axis, angle );
                    _this.object.up.applyQuaternion( quaternion );
                    _eye.applyQuaternion( quaternion );
	...
	function mousedown( event ) {
		...
		if ( _state === STATE.NONE ) {
		    ...
		    if (_this.controlMode == "auto")
	        {
		        _state = event.button;
	        }
		    else if (_this.controlMode == "rotate")
	        {
		        _state = STATE.ROTATE;
	        }
		    else if (_this.controlMode == "pan")
	        {
		        _state = STATE.PAN;
	        }
		}
		...
		this.addEventListener( 'mouseup', mouseup2, false );
    ...
    function mouseup2(event)
    {
        ...
        this.removeEventListener( 'mouseup', mouseup2 );
    }
	...
	function touchstart( event ) {
		...
		switch ( event.touches.length )
		{
			...
			case 1:
			    if (_this.controlMode == "auto" ||  _this.controlMode == "pan")
	            {
